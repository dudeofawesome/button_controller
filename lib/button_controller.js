...
var buttonFunc = function () {
	for (var i = 0; i < buttons.length; i++) {
		if (buttons[i].button.read() === 1 && !buttons[i].downLastTick) {
			if (buttons[i].clickCallback != null) {
				buttons[i].clickCallback();
			}
			buttons[i].timePressed = (new Date()).getTime();
		} else if (buttons[i].button.read() === 0 && buttons[i].downLastTick) {
			if (buttons[i].endClickCallback != null) {
				buttons[i].endClickCallback();
			}
			buttons[i].longClicked = false;
		} else if (buttons[i].button.read() === 1 && (new Date()).getTime() - buttons[i].timePressed >= longClickTime) {
			if (!buttons[i].longClicked) {
				console.log((new Date()).getTime() + " - " + buttons[i].timePressed + " >= " + longClickTime);
				if (buttons[i].longClickCallback != null) {
					buttons[i].longClickCallback();
				}
				buttons[i].longClicked = true;
			}
		}
