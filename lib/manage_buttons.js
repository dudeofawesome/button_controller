module.exports = {
	add: function (gpio, click, down, endClick) {
		var placeInArray = -1;
		for (var i = 0; i < buttons.length; i++) {
			if (buttons[i].button.number === gpio) {
				buttons[i] = new Button (gpio, click, down, endClick);
		...
		if (placeInArray === -1) {
			buttons.push(new Button (gpio, click, down, endClick));
			checkButtons.startCheckingButtons();
...
function Button (gpio, click, down, endClick) {
	this.number = gpio;
